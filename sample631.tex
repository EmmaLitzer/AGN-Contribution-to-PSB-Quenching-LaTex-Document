...
\shorttitle{AGN Contribution to PSB Quenching}
\shortauthors{Litzer et al.}
%%
...
\title{AGN Contribution to PSB Quenching}
...
\tablenum{1}
\tablecaption{Variables in Prospector Alpha\label{tab:vars}}
\tablewidth{0pt}
\tablehead{
\colhead{Variable} & \colhead{Description}
}
\startdata
agebins & Age of galaxy in bins for easier SFH \\
sfr\_fraction & ?? \\
z\_fraction & ??  \\
imf\_type & Chabrier (2003) \\
dust\_type & $\begin{tabular}[t]{@{}c@{}}Extinction curve for dust around old stars. 4: Kriek \& Conroy (2013) curve \end{tabular}$ \\
dust1 & Dust parameter describing the attenuation of young stellar light\\
dust1\_fraction & ?? \\
dust1\_index & $\begin{tabular}[t]{@{}c@{}}Power law index of the attenuation curve affecting stars younger than dust\_tesc corresponding to dust1\end{tabular}$\\
dust\_tesc & $\begin{tabular}[t]{@{}c@{}}Stars younger than dust\_tesc are attenuated by both dust1 and dust2, while  older stars are attenuated by dust2\end{tabular}$\\
duste\_gamma & $\begin{tabular}[t]{@{}c@{}}Specifies the relative contribution of dust heated at a radiation field strength of $U_{min}$ and dust heated at \\ $U_{min} < U \leq U_{max}$. Draine \& Li (2007)\end{tabular}$\\
duste\_umin & $\begin{tabular}[t]{@{}c@{}}Specifies the minimum radiation field strength in units of the MW value. Draine \& Li (2007)\end{tabular}$\\
duste\_qpah & $\begin{tabular}[t]{@{}c@{}}Specifies grain size distribution through the fraction of grain mass in PAHs. Draine \& Li (2007) \end{tabular}$ \\
% add_neb_continuum & nebular continuum component switch & Fixed\\
gas\_logz & $\begin{tabular}[t]{@{}c@{}}Log of the gas-phase metallicity; relevant for nebular emission model\end{tabular}$\\
gas\_logu & $\begin{tabular}[t]{@{}c@{}}Log of the gas ionization parameter; relevant for nebular emission model \end{tabular}$\\
\enddata
\tablecomments{Description of Prospector Alpha variables}
\end{deluxetable*}
...
\begin{deluxetable*}{lll}
\tablenum{2}
\tablecaption{AGN Variables in Prospector Alpha\label{tab:AGN vars}}
\tablewidth{0pt}
\tablehead{
\colhead{Variable} & \colhead{Description}  & \colhead{Type}
}
\startdata
add\_agn\_dust & ?? & Switch (On)\\
fagn & ?? & \\
agn\_tau & ?? & \\
\enddata
\tablecomments{Description of Prospector Alpha with AGN variables}
\end{deluxetable*}
...
\begin{deluxetable*}{lll}
\tablenum{3}
\tablecaption{Variables in Prospector Alpha: Switches\label{tab:switches}}
\tablewidth{0pt}
\tablehead{
\colhead{Switch} & \colhead{Description} & \colhead{Status}
}
\startdata
add\_igm\_absorption & Absorption from intergalactic medium & On\\
add\_agb\_dust\_model & AGB circumstellar dust model (Villaume 2014) & On\\
add\_dust\_emission & Draine \& Li 2007 dust emission model  \\
add\_neb\_emission & $\begin{tabular}[t]{@{}c@{}}Nebular emission model (continuum and line emission), based on \\ Cloudy models from Nell Byler\end{tabular}$  & On\\
nebemlineinspec & Flag to include the emission line fluxes in the spectrum & Off\\
\enddata
\tablecomments{Prospector Alpha Switches (excluding AGN switches)}
\end{deluxetable*}
...
\tablenum{4}
\tablecaption{Variables in Prospector Alpha: Values and Priors\label{tab:Values}}
\tablewidth{0pt}
\tablehead{
\colhead{Variable} & \colhead{Units} & \colhead{N} & \colhead{Prior} & \colhead{Init} & \colhead{IsFree}
}
% & & \\
\startdata
agebins & log(yr) & ncomp & & agebins\_init & Fixed\\
sfr\_fraction & & ncomp -1 & TopHat(mini=0, maxi=1) & 1/ncomp & Fixed\\
z\_fraction & & ncomp -1 & Beta(alpha=tilde\_alpha, beta=1, mini=0, maxi=1) & z\_fraction\_init & Free\\
imf\_type & & 1 & & 1 & Fixed\\
dust\_type & Index & 1 & & 4 & Fixed\\
dust1 & & 1 & TopHat(mini=0, maxi=6) & 1 & Fixed\\
dust1\_fraction & & 1 & ClippedNormal(mini=0, maxi=2, mean=1, sigma=0.3) & 1 & Free\\
dust1\_index & & 1 & TopHat(mini=-1.5, maxi=-0.5) & -1 & Fixed\\
dust\_tesc & log(Gyr) & 1 & & 7 & Fixed\\
duste\_gamma & & 1 & TopHat(mini=0, maxi=1) & 0.01 & Free\\
duste\_umin & & 1 & TopHat(mini=0.1, maxi=25) & 1 & Free\\
duste\_qpah & \% & 1 & TopHat(mini=0, maxi=10) & 3 & Free\\
gas\_logz & $log(\frac{Z}{Z_{\odot}})$ & 1 & TopHat(mini=-2, maxi=0.5) & 0 & Fixed \\
gas\_logu & & 1 & TopHat(mini=-4, maxi=-1) & -2 & Fixed\\
\enddata
\tablecomments{Prospector Alpha Variable values and priors}
\end{deluxetable*}
...
\begin{deluxetable*}{llllll}
\tablenum{5}
\tablecaption{Variables in Prospector Alpha: Values and Priors for AGN\label{tab:AGN Values}}
\tablewidth{0pt}
\tablehead{
\colhead{Variable} & \colhead{Units} & \colhead{N} & \colhead{Prior} & \colhead{Init} & \colhead{IsFree}
}
% & & & & & \\
\startdata
fagn & & 1 & LogUniform(mini=0.000316228, maxi=3) & 0.05 & Free\\
agn\_tau & & 1 & LogUnifrom(mini=5, maxi=150) & 10 & Free\\
\enddata
\tablecomments{Prospector Alpha AGN Variable values and priors}
\end{deluxetable*}
...
\author[0000-0002-7370-7731]{Emma Litzer}
\affiliation{University of Illinois at Urbana-Champaign \\
901 West Illinois Street, \\ Urbana, IL 61801} 

\author{Decker French}
\affiliation{University of Illinois at Urbana-Champaign \\
901 West Illinois Street, \\ Urbana, IL 61801} 
